...
<Note type="important">
  **Prerequisite setup is required before using `window.powerplatformAPI`:**
  ...
  1. Configure your Microsoft Entra app registration using the end-user guide: **[Entra App Registration Setup](/authentication/entra-app-registration)**.
  2. In your Power Platform ToolBox connection settings, populate the App Registration **Client ID**.
  3. Enable **Enabled for Power Platform API** in that connection when you want to call `window.powerplatformAPI`.
  4. In your tool's `package.json`, set `features.enabledForPowerPlatformAPI` to `true`.
...
`enabledForPowerPlatformAPI` declares that the tool uses this API; it does not enable the API for a connection. Users must still enable **Enabled for Power Platform API** for each connection the tool uses.
