...
export const sections = [
  ...
  { title: 'CSDL Metadata Document', id: 'csdl-metadata-document' },
...
## CSDL Metadata Document {{ anchor: true }}
  ...
  for (const action of actions) {
    if (action.getAttribute('Name') === actionName) {
      return {
        ...
        parameters: Array.from(action.querySelectorAll('Parameter')).map(p => ({
...
const whoAmIAction = findAction(xmlDoc, 'WhoAmI')
if (whoAmIAction) {
  console.log('WhoAmI action details:', whoAmIAction)
}
...
function findActionsByReturnType(xmlDoc, returnType) {
  ...
  actions.forEach((action) => {
    ...
    if (actionReturnType && actionReturnType.getAttribute('Type').includes(returnType)) {
...
<Note>
  **Pro Tip:** The CSDL document is large (1-5MB). Consider caching it if you need to reference it multiple times, or use specific metadata endpoints like `getEntityMetadata()` or `getEntityRelatedMetadata()` for targeted queries.
</Note>

<Note>
  **XML Namespaces:** The CSDL document uses XML namespaces. When querying, you may need to handle namespaces appropriately. The examples above use simple `querySelectorAll()` which works for element names, but for more complex queries, consider using namespace-aware XML parsing.
</Note>
