...
| `features.multiConnection` | Required with `features` | Must be `"required"`, `"optional"`, or `"none"` |
| `features.connectionRequirement` | ⚠️ optional | If present: must be `"required"` or `"optional"`; defaults to `"required"` |
| `features.minAPI` | ⚠️ optional | If present: must be a valid semantic version string |
| `features.enabledForPowerPlatformAPI` | ⚠️ optional | If present: must be `true` or `false` |
