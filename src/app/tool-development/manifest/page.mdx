...
Each function in the [API Reference](/tool-development/api-reference) includes an _Available since_ label showing the ToolBox version it was introduced in (where available). Set `minAPI` to the highest such version across all the functions your tool depends on.
