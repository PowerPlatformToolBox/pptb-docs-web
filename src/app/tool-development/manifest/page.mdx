...
Declare ToolBox-specific capabilities your tool needs. Omit this section entirely when your tool uses the default connection behavior and does not use the Power Platform API. When present, `multiConnection` is required.

| Field | Type | Values | Description | Sample |
|-------|------|--------|-------------|--------|
| `multiConnection` | `string` | `"none"` \| `"optional"` \| `"required"` | Connection cardinality: one connection, an optional second connection, or a required second connection | `"optional"` |
| `connectionRequirement` | `string` | `"required"` \| `"optional"` | Whether the tool needs any connection. Defaults to `"required"` | `"optional"` |
| `minAPI` | `string` | SemVer | Minimum ToolBox API version the tool requires | `"1.2.0"` |
| `enabledForPowerPlatformAPI` | `boolean` | `true` \| `false` | Opt the tool in to the Power Platform API. Set to `true` before using `window.powerplatformAPI` | `true` |
...
### Creating Connectionless Tools

Set `connectionRequirement` to `"optional"` when the tool can run without a Dataverse connection. Combine it with `multiConnection: "none"` to let the user open the tool with either zero or one connection.
...
For tools that can work without a connection but can also use multiple connections, set `connectionRequirement` to `"optional"` and `multiConnection` to `"optional"` or `"required"`. The tool can then open with zero, one, or two connections.
...
Each function in the [API Reference](/tool-development/api-reference) includes a **Requires vX.Y.Z** badge showing the minimum ToolBox version it needs (where applicable). Set `minAPI` to the highest such version across all the functions your tool depends on.
