...
export const sections = [
  ...
  { title: 'Rating Tools', id: 'rating-tools' },
  { title: 'Reporting a Concern', id: 'reporting-a-concern' },
  {
  ...
  { title: 'Working with Multiple Tools', id: 'working-with-multiple-tools' },
...
Organizations can add one or more private marketplace sources for internal tools. Private and built-in sources can be enabled together, or the built-in marketplace can be disabled after a private source is active.

<div className="not-prose mt-4">
  <Button href="/tool-installation/private-marketplace" variant="text" arrow="right">
    <>Set Up a Private Marketplace</>
  </Button>
</div>

## Rating Tools {{ anchor: true }}

You can rate tools directly from the desktop app without needing to sign in or navigate to the web:

1. Open the tool details page in the **Tools Marketplace** or from your **Installed Tools**.
2. View community ratings and average score for the tool.
3. Select your rating (1 to 5 stars) and submit your feedback.
4. Your rating is captured and submitted directly within the app.

<Note type="information">
  In-app ratings help the community identify great tools and provide valuable feedback to tool authors. Ratings are also one of the trust signals evaluated in the [Tool Maturity Model](/tool-development/maturity-model).
</Note>

## Reporting a Concern {{ anchor: true }}

If a tool appears to be spam, unsafe, inappropriate, or otherwise in violation of the PPTB community standards, you can flag it directly from the app using **Report a Concern**, available from the tool detail tab, the sidebar context menu, or the app menu.

<div className="not-prose mt-4">
  <Button href="/tool-installation/report-a-concern" variant="text" arrow="right">
...
## Working with Multiple Tools {{ anchor: true }}

When you have several tools open, you can keep two tools visible at the same
time using side-by-side view.

Use this to compare environments, copy values between tools, or monitor one
tool while working in another.

<div className="not-prose mt-4">
  <Button href="/tool-installation/side-by-side-view" variant="text" arrow="right">
    <>Open Side-by-Side View Guide</>
  </Button>
</div>
...
The first time you launch the tool, or when a tool update adds new permissions, you will see a permission consent dialog.
...
When permissions are added, the dialog highlights only the new permissions that
need your decision. Permissions you previously approved remain visible as
read-only context.

Additional details on permissions, please see the Tool Developer documentation [here](/tool-development/csp-configuration).

<Note type="important">
  If you decline, the tool won't load. When declining permissions added by an
  update, your existing consent record remains unchanged. If you accept, your
  choice is saved until the tool requests additional permissions.
</Note>
...
<div className="not-prose mt-6 flex gap-3">
  ...
  <Button href="/tool-installation/verified-tools" variant="text" arrow="right">
